...
def process_joystick_buttons(joystick, tts, animation, motion, battery_proxy, font, screen):
    global arm_mode
    ...
    # Get battery level and display it on the screen
    battery_level = battery_proxy.getBatteryCharge()  # Get battery level
    screen.fill((255, 255, 255))  # Clear the screen
    text = font.render("Battery: {}%".format(int(battery_level)), True, (0, 0, 0))  # Render battery level text
    screen.blit(text, (10, 10))  # Display text on the screen
...
def main():
    try:
        ...
        # Initialize battery proxy
        battery_proxy = ALProxy("ALBattery", IP, PORT)
    ...
    # Initialize Pygame UI for battery display
    screen = pygame.display.set_mode((640, 480))  # Set screen size
    pygame.display.set_caption("Norma Control")  # Set window title
    font = pygame.font.SysFont(None, 36)  # Set font for displaying text

    try:
        while True:
            process_joystick_input(joystick, motion)
            process_joystick_buttons(joystick, tts, animation, motion, battery_proxy, font, screen)
